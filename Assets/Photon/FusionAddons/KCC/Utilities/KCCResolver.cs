namespace Fusion.Addons.KCC
{
	...
	public sealed class KCCResolver
	{
		...
		public Vector3 CalculateBest(int maxIterations, float maxError)
		{
			...
			if (_size == 2)
			{
				Vector3 direction0 = _corrections[0].Direction;
				Vector3 direction1 = _corrections[1].Direction;
		...
		public Vector3 CalculateErrorDescent(int maxIterations, float maxError)
		{
			...
			while (_iterations < maxIterations)
			{
				...
				for (int i = 0; i < _size; ++i)
				{
					...
					float error = correction.Distance - Vector3.Dot(_targetCorrection, correction.Direction);
					if (error > 0.0f)
					{
						accumulatedError += error;
						_targetCorrection += error * correction.Direction;
					}
				}
