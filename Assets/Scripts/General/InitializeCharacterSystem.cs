...
namespace FPS_personal_project
{
    ...
    public partial struct InitializeCharacterSystem : ISystem
    {
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (physicsMass, mobaTeam, newCharacterEntity) in SystemAPI.Query<RefRW<PhysicsMass>, GameTeam>()
                         .WithAny<NewCharTag/*, NewMinionTag*/>().WithEntityAccess())
            {
                ...
                {
                    TeamType.A => new float4(0, 0, 1, 1),
                    TeamType.B => new float4(1, 0, 0, 1),
                    _ => new float4(1)
                };

                //ecb.SetComponent(newCharacterEntity, new URPMaterialPropertyBaseColor { Value = teamColor });
                ecb.RemoveComponent<NewCharTag>(newCharacterEntity);
